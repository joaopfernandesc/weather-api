...
describe 'WeatherController', type: :request do
  ...
  describe 'GET /weather' do
    ...
    context 'when "city" query parameter is present' do
      context 'and only one city is given' do
        let(:city) { 'page' }
      ...
      context 'and multiple cities are given' do
        let(:city) { 'page,nashville' }

        it 'should return only those weather records, which belong to that city' do
          expect(response.status).to eq(200)
          expected = [expected_responses[0], expected_responses[1], expected_responses[2]]
